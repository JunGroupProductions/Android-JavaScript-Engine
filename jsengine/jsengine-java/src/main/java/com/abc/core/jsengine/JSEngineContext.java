...
public final class JSEngineContext implements Closeable {
  ...
  private final Map<Class, JSEngineCoercion> JavaScriptToJavaCoercions = new LinkedHashMap<>();
  private final Map<Class, JSEngineCoercion> JavaToJavascriptCoercions = new LinkedHashMap<>();
  final Map<Method, JSEngineMethodCoercion> JavaScriptToJavaMethodCoercions = new LinkedHashMap<>();
  final Map<Method, JSEngineMethodCoercion> JavaToJavascriptMethodCoercions = new LinkedHashMap<>();
  private JSEngineInvocationHandlerWrapper invocationHandlerWrapper;
  ...
  public void setInvocationHandlerWrapper(JSEngineInvocationHandlerWrapper invocationHandlerWrapper) {
   ...
   */
  public synchronized <T> void putJavaScriptToJavaCoercion(Class<T> clazz, JSEngineCoercion<T, Object> coercion) {
   ...
   * @param <F>
   */
  public synchronized <F> void putJavaToJavaScriptCoercion(Class<F> clazz, JSEngineCoercion<Object, F> coercion) {
  ...
  public Object coerceJavaToJavaScript(Class clazz, Object o) {
    ...
    while (o instanceof JSEngineJavaObject) {
      Object coerced = ((JSEngineJavaObject)o).getObject();;
      if (o == coerced)
        break;
      o = coerced;
    }
  ...
  public Object coerceJavaScriptToJava(Class<?> clazz, Object o) {
    ...
    if (clazz.isInterface() && o instanceof JavaScriptObject) {
      ...
      if (lambda != null) {
        return Proxy.newProxyInstance(JSEngineJavaScriptObject.class.getClassLoader(), new Class[]{JSEngineJavaScriptObject.class, clazz},
        ...
        return Proxy.newProxyInstance(JSEngineJavaScriptObject.class.getClassLoader(), new Class[] { JSEngineJavaScriptObject.class, clazz }, handler);
      }
    }
  ...
  public synchronized void putJavaScriptToJavaMethodCoercion(Method method, JSEngineMethodCoercion coercion) {
    JavaScriptToJavaMethodCoercions.put(method, coercion);
    interfaceMethods.clear();
  }

  public synchronized void putJavaToJavaScriptMethodCoercion(Method method, JSEngineMethodCoercion coercion) {
  ...
  private static <T> T createMethodInterceptProxy(Class<T> clazz) {
    return (T)Proxy.newProxyInstance(clazz.getClassLoader(), new Class[] { clazz }, JSEngineContext::throwInvokedMethod);
  }
  ...
  private static Object coerceJavaToJavaScript(Map<Class, JSEngineCoercion> coerce, Object o, Class<?> clazz) {
    JSEngineCoercion coercion = coerce.get(clazz);
    if (coercion != null) {
      return coercion.coerce(clazz, o);
    }

    // check to see if there is a superclass converter (ie, Enum.class as a catch all).
    for (Map.Entry<Class, JSEngineCoercion> check: coerce.entrySet()) {
  ...
  private static Object coerceJavaScriptToJava(Map<Class, JSEngineCoercion> coerce, Object o, Class<?> clazz) {
    JSEngineCoercion coercion = coerce.get(clazz);
    if (coercion != null) {
      return coercion.coerce(clazz, o);
    }
  ...
  private JSEngineContext(boolean useQuickJS) {
    // coercing javascript string into an enum for java
    JavaScriptToJavaCoercions.put(Enum.class, (JSEngineCoercion<Enum, Object>) (clazz, o) -> {
      if (o == null)
        return null;
   ...
   * reporting.
   *
   * @throws JSEngineException if there is an error evaluating the script.
   */
  ...
  // this method should NOT be synchronized at the JSEngineContext level, so it never blocks a finalizer
